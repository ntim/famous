...
class Run(luigi.contrib.external_program.ExternalProgramTask):
    ...
    dz = luigi.FloatParameter(507.89 - 498.33425)  # mm
    nice_level = luigi.IntParameter(5, significant=False)
    n_particles = luigi.IntParameter(1000, significant=False)
...
class DzWavelengthSimulation(luigi.Task):
    ...
    def requires(self):
        ...
        task_kwargs = [dict(wavelength=wavelength, name=self.name, n_sipms=self.n_sipms) for wavelength in np.random.uniform(270, 1020, self.n_repititions)]

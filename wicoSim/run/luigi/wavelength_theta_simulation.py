...
class Run(luigi.contrib.external_program.ExternalProgramTask):
    ...
    n_particles = luigi.IntParameter(1000)
...
class Transmission(luigi.Task):
    ...
    def run(self):
        ...
        df = pd.DataFrame({"wavelength": [self.wavelength], "theta": [self.theta], "t": [trans]})
...
class WavelengthThetaSimulation(luigi.Task):
    name = luigi.Parameter("wavelength-theta-simulation")
    n_repititions = luigi.IntParameter(4096)
    ...
    def requires(self):
        ...
        deps = []
        i = 0
        for wavelength, theta in hammersley.plane(self.n_repititions, (200, 1000), (0, 36)):
            deps.append(Transmission(id=i, name=self.name, wavelength=wavelength, theta=theta))
            i += 1
    ...
    def output(self):
        return luigi.LocalTarget("./results/%s/wico-wavelength-theta-simulation.pkl" % self.name)


if __name__ == "__main__":
    luigi.run(main_task_cls=WavelengthThetaSimulation)
